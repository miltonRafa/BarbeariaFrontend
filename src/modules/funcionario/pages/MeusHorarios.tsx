...
} from "../../horario/services/horarioDisponivelService";
...
function MeusHorarios() {
  ...
  const horarioMinimo = dataHorarios === hoje ? horaAtualArredondada() : "";
  ...
  async function disponibilizarHorario() {
    try {
      ...
      if (`${dataHorarios}T${inicio}` < new Date().toISOString().slice(0, 16)) {
        alert("Não é possível disponibilizar um horário que já passou.");
        return;
      }
      ...
      const mensagem = axios.isAxiosError(error)
        ...
        : null;
      alert(mensagem ?? "Erro ao disponibilizar horário.");
    }
  }
          ...
          {[
            ...
            .map(([presetInicio, presetFim]) => {
              const inicioAjustado =
                horarioMinimo && presetInicio < horarioMinimo
                  ? horarioMinimo
                  : presetInicio;

              return [inicioAjustado, presetFim];
            })
            .filter(([presetInicio, presetFim]) => presetInicio < presetFim)
            .map(([presetInicio, presetFim]) => (
              <button
...
function horaAtualArredondada() {
  ...
  if (resto !== 0) {
    agora.setMinutes(minutos + (5 - resto));
  }
